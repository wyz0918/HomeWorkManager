...
def home():
    ...
    if student_additional_info==None:
        student_additional_info={"username": None, 'university': None, 'major': None,'enrollment_year':None,'college':None,'birthday':None}
    else:
...
def changemsg():
    ...
    if data['username']!='' and data['university']!=''and data['major']!='':
        ...
        code='100'
        if student_additional_info==None:
            s_a_i=AdditionalStudentInfo(user.id,data['enrollment_year'],data['university'],data['college'],data['major'],data['birthday'])
...
def summit_homework():
    if request.method == 'POST':
        ...
        if c!=None:
            ...
            cp=Completion(g.user.id,int(data['work_id']),None,time.strftime("'%Y.%m.%d'"),None,None,filename)
            db.session.add(cp)
            db.session.commit()
