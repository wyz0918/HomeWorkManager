...
def parse_opts():
    ...
    parser.add_argument('-p', metavar='rackposition', type=str, choices=['left', 'right', 'front', 'interior', 'back'],
                        help='rackspace detailed position')
...
def isup(host):
    ...
    try:
        ...
        print
        "Failed to connect to host {0} port 22: Network is unreachable".format(host)
        return False
    return True
...
class GetServerInfo(object):
    ...
    def get_server_type(self):
        ...
        ec2_sign = run(
            """curl http://169.254.169.254/latest/meta-data/hostname --connect-timeout 1 |egrep -wq 'ec2.internal|compute.internal'""")
    ...
    def get_disk(self):
        disk = sudo(
            """fdisk -l 2>/dev/null |grep -v "/dev/mapper" |grep "Disk /dev" |awk '{print $2" "$3" "$4}'|grep -Ev '^$|^doesn' |sort |xargs""")
        return disk
    ...
    def get_rst_on(self):
        ...
        vm_uuid = sudo("""xe vm-list |grep -B1 -w %s |awk '{if ($1 == "uuid") print $NF}'""" % (opts['hostname']),
                       hostname=xs_pool_master)
        rst_uuid = sudo("""xe vm-param-get uuid={0} param-name=resident-on""".format(vm_uuid), hostname=xs_pool_master)
        rst_name = sudo(
            """xe vm-list params |egrep 'name-label|resident-on' |grep -B1 %s |grep -w "Control domain" |awk -F ": " '{print $NF}' |cut -d. -f1""" % (
                rst_uuid), hostname=xs_pool_master)
        return rst_name
...
def sum_info():
    ...
    print
    "CPU:         " + cpu_info['cpu_cores'] + " Cores, " + cpu_info['cpu_type']
...
def read_db(info):
    ...
    if not object_id:
        print
        "Object:{0} does not exist".format(info['hostname'])
        return False
    ...
    if not item:
        print
        "Object:{0} does not have location info".format(info['hostname'])
        return False
    ...
    if not location_name or not row_name or not rack_name:
        ...
        return False
    print
    "RACKSPACE:   {0}:{1}:{2}:{3}".format(location_name, row_name, rack_name, unit_no)
...
def update_db(info):
    ...
    if req.status_code != requests.codes.ok:
        print
        "Failed to create object: {0}".format(info['hostname'])
        return False
    else:
        print
        "OK - Created object: {0}".format(info['hostname'])
    ...
    if not os_release_key:
        print
        "Failed to get object_type_id, please add '{0}' to 'Configuration - Dictionary - Server OS type'.".format(
            info['os_release'])
        return False
...
&12_attr_id=24&12_value=&13_attr_id=10005&13_value={9}&14_attr_id=25&14_value=&num_attrs=15&object_comment=&submit.x=15&submit.y=13""" \
            .format(object_id, object_type_id, info['hostname'], info['cpu_cores'], quote_plus(info['disk']),
                    info['fqdn'],
    ...
    if info['server_type'] not in ["EC2"]:
        ...
        for i in nics:
            ...
            if nic_name.isalnum():
                ...
                if req.status_code != requests.codes.ok:
                    print
                    "Failed to update ip pool for {0}:{1}".format(nic_name, nic_addr)
                    return False
                print
                "OK - Updated ip pool for {0}:{1}".format(nic_name, nic_addr)
...
def update_rack(info, object_id):
    ...
    if opts['rackspace']:
        ...
        if not atom:
            print
            "The rackspace is not correct"
            return False

            # get rack_id
        for item in db.query(
                "select * from Rack where name = '{0}' and location_name = '{1}' and row_name = '{2}'".format(rack,
                                                                                                              colo,
                                                                                                              row)):
            rack_id = item.id
        if not rack_id:
            print
            "Failed to get rack_id"
            return False
        ...
        if req.status_code != requests.codes.ok:
            print
            "Failed to update rackspace"
            return False
        print
        "OK - Updated rackspace"
...
def delete_object(info):
    ...
    if object_id:
        ...
        if req.status_code != requests.codes.ok:
            print
            "Failed to delete the existing object: {0}".format(info['hostname'])
            return False
        else:
            print
            "OK - Deleted the existing object: {0}".format(info['hostname'])
...
def list_object(info):
    ...
    # get rack_id
    for item in db.query(
            "select * from Rack where name = '{0}' and location_name = '{1}' and row_name = '{2}'".format(rack, colo,
                                                                                                          row)):
        rack_id = item.id
    if not rack_id:
        print
    ...
    for object_id in sorted(list(set(object_id_list))):
        ...
        print
        "{0}: {1}".format(object_type_name, object_name)
...
if __name__ == '__main__':
    ...
    if not opts['list'] and not opts['delete'] and not opts['blank'] and not opts['switch'] and not opts['security'] \
        ...
        if hostup:
            ...
            "========================================"
            print
            "Getting informations from '{0}'...".format(opts['hostname'])
            print
            "========================================"
            info = sum_info()
        ...
        if opts['list']:
            ...
            "========================================"
            print
            "Getting objects in rackspace:'{0}'...".format(opts['hostname'])
            print
            "========================================"
            list_object(info)
    ...
    if opts['delete']:
        ...
        "========================================"
        print
        "Deleting Object: '{0}' from DB...".format(opts['hostname'])
        print
        "========================================"
        delete_object(info)
